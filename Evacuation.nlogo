...
to update-goals
  ...
  set target min-one-of patch-set visible-exits [distance myself]
  ...
  if target != nobody [
    set immediate-target min-one-of neighbors with [ (pcolor = ground-color or pcolor = door-color) and not any? turtles-on self ] [distance [target] of myself]
  ]
end

to-report desirability-weight [ p ]
  let wall-proximity [count neighbors with [ pcolor = wall-color ]] of p
  let crowdedness [count turtles in-radius 1] of p
  let distance-to-target ifelse-value (target != nobody) [[distance [target] of myself] of p] [0]

  let wall-weight 2
  let crowdedness-weight 1
  let dist-weight 1

  let weight (wall-weight * (wall-proximity) + crowdedness-weight * (crowdedness) + dist-weight * (distance-to-target))
  report weight
end
...
; turtle reporter
to-report visible? [ p ] ; patch
  let heading-to-p towards p
  let viewing-angle 200 ; including far peripheral view

  let within-viewing-angle heading-difference heading heading-to-p < (viewing-angle / 2)
  ifelse within-viewing-angle [
...
to-report obstacles-blocking? [ p ]
  ...
  while [[pcolor] of patch-here = ground-color and patch-here != p] [ ; TODO people should block visibility, too.
    fd 1
  ]
